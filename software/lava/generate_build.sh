#!/bin/bash
# This script generates and builds everything for the Colfax build
...
main()
{
    ...
    if [ -d toolchain/gcc-arm-none-eabi-5_4-2016q3 ]; then
        echo "Toolchain exists"
    else
        echo "Extracting toolchain"
        cd toolchain
        tar -xf gcc-arm-none-eabi-5_4-2016q3-20160926-linux.tar.xz
        cd ..
    fi
...
function create_build()
{
    ...
    if [ ! -z "${3}" ]; then
        ...
        cmake_args="-DCMAKE_TOOLCHAIN_FILE:PATH=${toolchain_file} -DCMAKE_BUILD_TYPE=Release"
    fi
