...
void CySysClkImoStop(void)
{
    ...
    *******************************************************************************/
    void CySysClkImoEnableUsbLock(void)
    {
            ...
            uint32 CySysClkUsbCuSortTrim = ((4u < (CY_SFLASH_M0_SMR_3_REG & CY_SFLASH_M0_SMR_3_MASK)) ? 1u : 0u);
            uint32 i;
        ...
        /* Set oscillator interface control port to USB */
            ...
            if (48u == freq)
            {
                CY_SYS_CLK_IMO_TRIM1_REG = (CySysClkUsbCuSortTrim) ? (uint32)CY_SFLASH_CU_IMO_TRIM_USBMODE_48_REG :
                                                                     (uint32)CY_SFLASH_IMO_TRIM_USBMODE_48_REG;
            }
            else if (24u == freq)
            {
                CY_SYS_CLK_IMO_TRIM1_REG = (CySysClkUsbCuSortTrim) ? (uint32)CY_SFLASH_CU_IMO_TRIM_USBMODE_24_REG :
